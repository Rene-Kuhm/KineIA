...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        version = tuple(int(part) for part in client.info().version.split(".")[:2])
        if version < (1, 18):
        ...
        print(json.dumps({"operation": args.command, "status": "error",
                          "error": "Hybrid migration command failed"}, sort_keys=True))
        return 2
